...
(function() {
    ...
    extendNative = function(to, from) {
        ...
        try {
            ...
            if (null === to[key] && "function" !== typeof val) {
    ...
    Offline = {},
    (null === Offline.options || Offline.options === undefined) && (Offline.options = {}),
    ...
    "function" == typeof window.addEventListener && window.addEventListener("online",
        function() {
            return setTimeout(Offline.confirmUp, 100);
        }, false),

    "function" == typeof window.addEventListener && window.addEventListener("offline",
        function() {
    ...
    Offline.markUp = function() {
        return Offline.trigger("confirmed-up"),
        "up" !== Offline.state ? (Offline.state = "up",
        Offline.trigger("up")) :void 0;
    },


    Offline.markDown = function() {
        return Offline.trigger("confirmed-down"),
        "down" !== Offline.state ? (Offline.state = "down",
    ...
    Offline.on = function(event, handler, ctx) {
        ...
        if (events = event.split(" "), events.length > 1) {
            ...
            for (results = [], j = 0, len = events.length; len > j; j++)
            {
                e = events[j];
        ...
        return (null === handlers[event] || undefined === handlers[event]) && (handlers[event] = []), handlers[event].push([ ctx, handler ]);
    },

    Offline.off = function(event, handler) {
        var _handler, ctx, i, ref, results;
        if ( (null !== handlers[event]) && (undefined !==handlers[event]) ) {
            if (handler) {
    ...
    Offline.trigger = function(event) {
        ...
        if ( (null !== handlers[event]) && (undefined !== handlers[event]) ) {
            for (ref = handlers[event], results = [], j = 0, len = ref.length; len > j; j++)
            {
                ref1 = ref[j];
                ctx = ref1[0];
                handler = ref1[1];
    ...
    checkXHR = function(xhr, onUp, onDown) {
        ...
        null === xhr.onprogress ?
            ( _onerror = xhr.onerror,
                ...
                {
                    return onDown(),
                    "function" == typeof _onerror ? _onerror.apply(null, arguments) :void 0;
                }, 

                ontimeout = xhr.ontimeout, xhr.ontimeout = function() {
                    return onDown(),
                    "function" == typeof _ontimeout ? _ontimeout.apply(null, arguments) :void 0;
                },
                ...
                xhr.onload = function() {
                    return checkStatus(),
                    "function" == typeof _onload ? _onload.apply(null, arguments) :void 0;
                }
            ) :

            (_onreadystatechange = xhr.onreadystatechange,
                xhr.onreadystatechange = function() {
                    return 4 === xhr.readyState ? checkStatus() :
    ...
    Offline.checks = {}, Offline.checks.xhr = function() {
        ...
        (null !== xhr.timeout && undefined !== xhr.timeout) && (xhr.timeout = Offline.getOption("checks.xhr.timeout")),
    ...
    Offline.confirmUp = Offline.confirmDown = Offline.check, Offline.onXHR = function(cb) {
        ...
        extendNative(window.XMLHttpRequest, _XMLHttpRequest), (null !== window.XDomainRequest && undefined !== window.XDomainRequest)?
            (
    ...
    init = function() {
        return Offline.getOption("interceptRequests") && Offline.onXHR(function(arg) {
            var xhr;
            return xhr = arg.xhr, xhr.offline !== false ? checkXHR(xhr, Offline.markUp, Offline.confirmDown) :void 0;
        }),
...
function() {
    ...
    rc = Offline.reconnect = {}, retryIntv = null, reset = function() {
        var ref;
        return (null !== rc.state && undefined !== rc.state) && "inactive" !== rc.state && Offline.trigger("reconnect:stopped"),
        rc.state = "inactive",
        rc.remaining = rc.delay = null != (ref = Offline.getOption("reconnect.initialDelay")) ? ref :3;
    },
    ...
    up = function() {
        return (null !== retryIntv && undefined !== retryIntv) && clearInterval(retryIntv), reset();
    },
...
function() {
    ...
    held = [], waitingOnConfirm = false, holdRequest = function(req) {
    ...
    setTimeout(function() {
    return Offline.getOption("requests") !== false ? (Offline.on("confirmed-up", function() {
        return waitingOnConfirm ? (waitingOnConfirm = false, clear()) :void 0;
    }),
    ...
    Offline.onXHR(function(request) {
        ...
        return xhr = request.xhr, async = request.async, xhr.offline !== false &&
        ...
        ) ? null === xhr.onprogress ? (xhr.addEventListener("error", hold, false),
...
function() {
    ...
    render = function() {
        ...
        null != button.addEventListener ? button.addEventListener("click", handler, false) :button.attachEvent("click", handler)),
    ...
    (null !== document.addEventListener && undefined !== document.addEventListener) ?

    document.addEventListener("DOMContentLoaded", init, false) :(_onreadystatechange = document.onreadystatechange,
